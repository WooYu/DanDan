...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void initView() {
        ...
        iv_dgvip = (ImageView) findViewById(R.id.iv_dgvip);
...
//    	String    str    =    formatter.format(curDate); 
        ...
        if (timestamp != null) {
            if (position == 0) {
                ...
                if (prevMessage != null && DateUtils.isCloseEnough(message.getMsgTime(), prevMessage.getMsgTime())) {
                    timestamp.setVisibility(View.VISIBLE);//原来是隐藏的
                    timestamp.setText(DateUtils.getTimestampString(new Date(message.getMsgTime())));//后加的
                } else {
                    timestamp.setText(DateUtils.getTimestampString(new Date(message.getMsgTime())));
                    timestamp.setVisibility(View.VISIBLE);
                }
        ...
        //这个if和最下面的else是一对的控制8001自定义的消息样式
//        if(!message.getUserName().equals("8001")){
        if(message.direct() == Direct.SEND){
            EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            EaseUserUtils.setUserGbVip(context, EMClient.getInstance().getCurrentUser(), iv_dgvip);
        }else{
            EaseUserUtils.setUserAvatar(context, message.getFrom(), userAvatarView);
            EaseUserUtils.setUserGbVip(context, message.getFrom(), iv_dgvip);
...
//        }else{
//        	//自定义8001的发送效果和最上面的if是一起的
//      	  System.out.println("不设置头像和昵称"+message.getUserName());
//        }
